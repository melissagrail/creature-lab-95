...
def identity():
    lib = library()
    return dict(format=FORMAT, rules=lib.cr_version(), observations=lib.cr_observation_version(),
                content=lib.cr_content_hash(), observation_size=OBS_SIZE, hidden=HIDDEN,
                action_codec='opponent-local-tanh-v1')
...
def save_checkpoint(path, policy, **extra):
    ...
    torch.save(dict(identity=identity(), policy=policy.state_dict(), **extra), temp)
    temp.replace(path)
...
def distribution(policy, obs, memory, raw=None, ability=None, deterministic=False):
    mean,logits,value,hidden=policy(obs,memory)
...
class ArenaBatch:
    def __init__(self,count,seed=7,scenarios=None):
...
def cases(count,seed=900_000_000):
    return [(i%40,(i%40+17+7*(i//40))%40,(i//40+i%3)%6,i%3,
             seed+i,(i+i//40)%2,(i//6)%4) for i in range(count)]

@torch.no_grad()
def evaluate(policy,count=80,seed=900_000_000,opponent='scripted'):
    scenarios=cases(count,seed); env=ArenaBatch(min(32,count),seed,scenarios)
    ...
    try:
        while len(results)<count:
            ...
            motion,ability,_,_,_,memory,_=distribution(policy,torch.from_numpy(obs),memory,deterministic=True)
            acts=env.teacher()
