...
| 3582:3622 | global[40] | Weather, layout, time, objectives, ending state and both actors' development limits |
| 3622:3628 | mask[6] | No-op plus five legal-action indicators |
...
## Footwork and campaign in rules 8

Rules 8 adds velocity-relative footwork recovery/costs, reserve-dependent lateral drive and finite cornering acceleration. The existing observation schema contains all required physical state; no new hidden effort accumulator is introduced. Global column 30 continues to report the actual current regeneration rate. `Features.spent` includes movement energy as well as accepted casts. Use `Started` events to count abilities. See [the exact footwork contract](alpha/footwork.md).

The forty directional animation atlases and campaign presentation are outside the duel. `creature::campaign` is a separate C++ progression/scenario/save layer. Its `TINISAV2` save envelope is not a duel snapshot. Campaign parties, trust, remedies and between-round resets do not appear as unannounced training rewards or policy inputs. `initialize_round` produces an ordinary fully observable World using the existing terrain, wind and status fields. Automatic campaign trajectory ingestion and online learning are not implemented.

Keep a separate recurrent state for each fresh duel. Campaign personality input combines a preset with a small stored-seed variation. Playing never edits weights. The optional training tools still train against headless duels, so campaign-specific scenario/party curricula are future work.

## Development and notes in rules 9
