...
CI includes CMake core tests on Linux/macOS/Windows and Linux viewer/FFI/sanitizer checks. Cross-platform claims should be conditioned on those jobs passing; the local x86-64 comparison uses Rosetta, not a separate Windows machine. There is no completed RL training run or measured learning curve in this repository.
