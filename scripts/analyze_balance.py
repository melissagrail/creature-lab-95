...
wins=[0.]*40;games=[0]*40;pair_w=[[0.]*40 for _ in range(40)];pair_n=[[0]*40 for _ in range(40)];reasons={};total=0;ticks=[];seat0=0;overflow=0;weather=[[0.,0] for _ in range(3)];arena={}
for r in csv.DictReader(source.open()):
 ...
 weather[int(r['weather'])][0]+=v;weather[int(r['weather'])][1]+=1
 ag=arena.setdefault(int(r['arena']),[0.,0]);ag[0]+=v;ag[1]+=1
...
summary=dict(matches=total,per_species=games[0],min_rate=min(rates),max_rate=max(rates),seat_a_win_rate=seat0/total,mean_seconds=sum(ticks)/len(ticks)/30,median_seconds=sorted(ticks)[len(ticks)//2]/30,reasons=reasons,overflow=overflow,metadata=meta,arenas={k:dict(matches=v[1],seat_a_win_rate=v[0]/v[1]) for k,v in sorted(arena.items())},source_sha256=hashlib.sha256(source.read_bytes()).hexdigest(),engine_sha256=hashlib.sha256((ROOT/'src/sim.cpp').read_bytes()).hexdigest(),species=[dict(id=i,name=names[i],wins=wins[i],games=games[i],rate=rates[i]) for i in range(40)])
prefix.with_suffix('.json').write_text(json.dumps(summary,indent=2)+'\n')
with prefix.with_name(prefix.name+'-matrix.csv').open('w') as f:
 out=csv.writer(f,lineterminator="\n");out.writerow(['species']+names)
 for name,row in zip(names,matrix):out.writerow([name]+[f'{v:.4f}'for v in row])
lines=['# Scripted balance evidence','',f'**{total:,} matches; {games[0]:,} games per species; {pair_n[0][1]} games per unordered matchup.** Every unordered pairing is run in both seats. This run uses {meta.get("seeds", "unknown")} seeds across {len(arena)} arenas and {sum(bool(v[1]) for v in weather)} weather states; a complete four-style pass requires {meta.get("arena_count",len(arena))*12} seeds. The species share a kit-aware heuristic; these are not independently trained or optimized policies.','',f'Aggregate species win rates: **{min(rates):.1%}–{max(rates):.1%}**. Side A wins: **{seat0/total:.2%}**. Mean fight: **{summary["mean_seconds"]:.1f}s**; median **{summary["median_seconds"]:.1f}s**. Pool overflows: **{overflow}**.','',f'Endings: KO {reasons.get(1,0):,}, control {reasons.get(2,0):,}, time limit {reasons.get(3,0):,}.','', 'These aggregate rates are an alpha smoke-test gate, not a claim of competitive balance. Scenario outcomes are correlated, so a binomial confidence interval would overstate the strength of this evidence. Fresh-seed holdout varies starting jitter; it does not constitute an independent policy-population holdout. The worst pairings below remain explicit human/RL playtest targets.','', '| Species | Win rate | Games |','|---|---:|---:|']
