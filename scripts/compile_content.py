...
FIELDS='startup active recovery cooldown cost damage range radius speed impulse burn haste pierces_evasion min_range lifetime period shots spread slow root silence poison wound mark shield heal guard cleanse drain bonus_mark execute health_cost bounces returning pierce move_start move_active move_recovery'.split()
...
for s in species:
 lines.append('{'+','.join(json.dumps(s[k]) for k in ['name','role','identity'])+','+','.join(str(s[k]) for k in ['hp','speed','radius','regen','mass'])+','+s['passive']+','+str(s['preferred_range'])+',{'+','.join(str(s['id']*4+j) for j in range(4))+'},{'+','.join(map(str,s['axes']))+'},'+','.join(str(s[k]) for k in ['turn_degrees','strafe','backward','acceleration','braking','dodge_speed'])+'},')
lines+=['}};','}']
cpp='\n'.join(lines)+'\n'
